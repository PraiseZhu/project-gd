...
| `execution-plus-code` | 同时有 execution artifact + code diff | **outcome before code** — 先 H4b validator，后 code review |
...
**Suite-mode Serial Contract（lock_revision=18，MANDATORY）**：

`/gd review plan` 在同一会话内对多个 plan 文件套件执行时（suite-mode），必须遵守：

1. **Single controller 串行**：所有 Codex bridge 调用必须串行，不得由 Claude 子 agent 并发执行。禁止以下模式：
   - "派 3 个子 agent 并行 review 套件各部分"
   - 任何形式的 concurrent `gd-codex-bridge-review.py run-bridge` 调用

2. **Runtime enforcement**：`gd-codex-bridge-review.py` 内置 lock sentinel（`/tmp/gd-codex-bridge.lock`）：若 600s 内已有另一个 bridge 进行中，当前调用返回 `SUITE_CONCURRENT_BRIDGE` 并以非成功退出，不进入 Codex transport。

3. **外层 timeout 禁止**：不得用 `timeout 120` 等外层命令包裹 bridge 调用；bridge 内部自带 timeout，外层包装会导致 Codex 真实 raw 结果被误判为 timeout。

4. **Final state 只读 aggregate-final.json**：suite 完成后，parent close gate 输入必须是 `aggregate-final.json`（带 review_contract_hash 的新格式），不得使用无 contract hash 的旧版 `aggregate.json`。

5. **Task packet metadata 标准（Claude 与 Codex 共用）**：Claude self-review 和 Codex cross-review 均须按 `prompts/gd-review-standard.md` 验证 task packet 完整性，包括：`forbidden_paths` / `required_context` / `deliverables` / `owned_paths`。缺少任何一项视为 REQUIRES_CHANGES。
...
- 支持两种 execution_mode：
  - `human_exec` — 主 agent 手动执行（Plan 5 v5 上线，未变）
  - **`agent_exec`** — child executor 子 agent 派发执行（Plan H lock_revision=6 H3 引入）
- 前置 artifact（任一缺失 → `blocked_missing_artifact`）：
...
| Codex cross-review code | Plan 6 | `pending_future_plan` |
