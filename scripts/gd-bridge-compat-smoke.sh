...
# Test 3 & 4: G2 — execution kinds WITHOUT any flag auto-infer --compat-v1 (must PASS)
for kind in execution_outcome combined; do
  if python3 "$BRIDGE" parse-transport \
      --kind "$kind" --target "smoke-target" \
      --raw-result "$RAW_FILE" --out "$OUT_FILE" 2>/dev/null; then
    echo "  PASS: $kind without explicit flag auto-infers compat-v1 (G2)"
    PASS=$((PASS+1))
  else
    echo "  FAIL: $kind should auto-infer compat-v1 and pass v1 raw"
    FAIL=$((FAIL+1))
  fi
done

# Test 5 & 6: explicit --no-compat-v1 MUST reject v1 raw for execution kinds
for kind in execution_outcome combined; do
  if python3 "$BRIDGE" parse-transport \
      --kind "$kind" --target "smoke-target" \
      --raw-result "$RAW_FILE" --out "$OUT_FILE" \
      --no-compat-v1 2>/dev/null; then
    echo "  FAIL: $kind --no-compat-v1 should reject v1 raw but passed"
    FAIL=$((FAIL+1))
  else
    echo "  PASS: $kind --no-compat-v1 correctly rejects v1 raw"
    PASS=$((PASS+1))
  fi
done
