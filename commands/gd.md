---
description: Claude-first /gd Goal-Driven 多 Agent 主入口。当前阶段（Plan H lock_revision=16）接四阶段入口 + /gd plan dispatch contract + /gd review unified router + /gd execute agent_exec contract；multi-agent planning dispatch（Plan 4 + H1，contract introduced at revision=2）★ dispatch map build + capability probe + manual_packet fallback + planning dispatch log；/gd review plan dual review（H2a，contract introduced at revision=3；partial_completed — Codex unavailable fail-closed path live, happy path pending W2）★ Claude self-review + Codex cross-review + merge + bounded auto-fix loop ≤3 rounds；revision=4 = anti-fill token externalization + CAPABILITY_STATUS runtime override footnote；revision=5 = lock_revision metadata sync after Codex cross-review；revision=6 = /gd execute agent_exec contract introduced (H3) — capability probe + execution_dispatch_ledger + child prompt builder + path audit；revision=7 = /gd review unified router introduced (H2b) — auto-detect 5 review target types (plan-only / execution-only-no-code / code-only / execution-plus-code / no-artifact) and dispatch to gd-detect-review-target + H4b gd-validate-execution-outcome (outcome first) + H2a gd-review-merge-and-fix-loop (plan)；revision=8 = drift reconciliation (plan4-drift-reconciliation)；revision=9 = Plan 4 live dispatch contract v1.1 (runtime binding anchors + probe.json schema + proof_mode tracking + source_dispatch_map_hash cross-check)；revision=10 = Plan 4 round 2 P1 fixes — execute-agent-exec strict runtime binding anchor + --strict-live-proof on all runtime validator calls + cross-validation entries (gd-validate-runtime-strict-binding / gd-run-live-dispatch-fixture-matrix / gd-validate-live-dispatch-smoke)；revision=11 = Plan 4 round 3 P1/P2 fixes — runtime_proof.probe_artifact_ref REQUIRED (non fail_closed) + SHA-256 hex format check on source_dispatch_map_hash + matrix smoke wired to gd-validate-live-dispatch-smoke against fixtures/live-dispatch-smoke (real hashes) + dim_probe full schema (enum/type/additionalProperties/date-time)；revision=12 = Plan 4 round 4 P1/P2 fixes — extracted scripts/gd-validate-probe.py as canonical single source of truth for probe schema; live-dispatch-smoke delegates probe enforcement to it (full enum/type/additionalProperties/format/conditional null); fail_closed_no_dispatch live smoke now allows --dispatch-map omission and requires non-empty fallback_reason (real "no dispatch" semantics)；revision=13 = Plan 5 v3 review-router hardening — gd-review-router.py unified state machine (4 modes: fixture/detect_only/live_dry_run/live) + gd-validate-route-report.py canonical schema SSOT (validator_signature Q2) + review-router runtime binding anchor (stage=review-router) + gd-validate-runtime-strict-binding.py config-driven extension + Q1 singleton check + Q3 GD_REVIEW_ROUTER_INVOCATION_ID side-door prevention + fixtures/review-router-v2/ (10 fixtures incl. 4 MUST FAIL) + fixture migration v1→v2；revision=14 = Plan 8 v4.1 review template split + v2 contract — gd_review_contract.py SSOT (REVIEW_KIND_ENUM 4 values / REVIEW_TARGET_KIND_ENUM 6 values / TEMPLATE_KIND_ENUM / DECISION_ENUM / TARGET_ROLE_ENUM) + gd-review-result-v2.schema.json (13 allOf: 5 base + 4 review_kind→target_kind + 4 review_kind→template_kind) + gd-validate-review-handoff.py (schema+path+sha256+SSOT consistency) + gd-emit-review-handoff.py (producer with self-validation) + gd-validate-combined-review.py (CVF-1/CVF-2/CVF-3 combined review emission rules) + gd-validate-review-contract-drift.py (AST-based definitional-literal SSOT drift scanner) + gd-codex-bridge-review.py v2 parse-transport (4 kinds) + compat-v1 explicit gate (--compat-v1 required; auto-detect/env-var/silent-fallback all false)；execution-only/plus-code cross-review 已在 revision=20 接入；code-only Codex sidecar 仍 pending Plan 6；revision=15 = Plan 9 Phase B production smoke 通过（complete_with_live_parallelism, 2026-05-15）+ Plan 10-lite terminal /gd live command install；revision=16 = Plan 1 review-chain correctness — live plan route via _consume_and_merge + aggregate v2 manifest batch + parent close gate stale/unbound/constrained blocking。
---
...
| `code-only` | 仅 code diff 无 execution artifact | 静态字段校验（execution-review template 派生） |
...
**lock_revision=7 install**：H2b 写 commands/gd.md → record `revision: 7 plan_ref: h2b-command-router` → 跑 `bash scripts/check-gd-command-parity.sh`。
...
- code-only target **必须**经过 1-2 个 review child agent（via router child review contract）；`LOCAL_STATIC_ONLY` / `pending_future_plan` / `static-only` 是 `closure_ineligible`，不得 APPROVED
- Codex code sidecar cross-review（pure code diff）属 Plan 6 pending；当前 code-only 必须有 review child 但 child 基于 Claude review；缺 child review ledger → `CLOSURE_INELIGIBLE: missing_child_review_ledger`，exit 1
...
| Multi-agent execution dispatch | Plan 5 | `local_only` human_exec ✓；agent_exec `pending_future_plan` |
...
| Codex cross-review code-only (code diff sidecar) | Plan 6 | `pending_future_plan` |
