...
| `code-only` | 仅 code diff 无 execution artifact | upstream quality gate + Codex bridge kind=`code_diff` cross-review + route validator + child review ledger（L1L3 补齐）；codex_review_status=completed + 有效 child ledger 才 APPROVED |
