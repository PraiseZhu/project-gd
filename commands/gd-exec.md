---
description: GD L3 · 执行（等价 /gd execute）。agent_exec 子 agent 化执行 + dispatch ledger + path audit。
---

# /gd-exec — 等价 `/gd execute`

`/gd execute` 的顶层快捷别名，行为完全等价。（中文别名 `/gd-执行` 等价，二选一即可。）
