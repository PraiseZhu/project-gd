...
| 档位 | 命令 | 它是干嘛的 | 什么时候用 |
|------|------|-----------|-----------|
| **L1** | `/review1` | 最轻量的"第二意见" | 拿不准某个判断时，让 Codex 给一段独立分析 |
| **L2** | `/review2` | 中量级的 Codex 审查工作台，支持审计划 / 审代码 / 审执行结果 | 有一份具体的计划、代码或执行结果，要 Claude 和 Codex 交叉审核时 |
| **L3** | `/gd` | 全链路——从写计划一路管到审代码，四阶段、全程双模型把关、留全套证据 | 管理一个完整任务的生命周期；或快速搓出 MVP（产出完整“计划包”：Master Plan = 总计划 + 各步骤详细计划，四阶段含多 agent 编排） |

### L3 `/gd` 的四个阶段

一个完整任务从写计划到审代码分四步，每步一个命令。下表也标清**每步靠什么机制**（机制本身详见下方「六大机制」）：

| 阶段 | 命令 | 这一步做什么 | 关键机制 |
|------|------|-------------|---------|
| **① 计划** | `/gd-plan` | 把一句话目标铺成完整「计划包」（总计划 + 分步计划 + 任务包），每份钉死目标、圈定可碰文件 | **多 agent 编排**——主 AI 派 1-2 个分身并行起草，自己只汇总把关 |
| **② 审计划**（动手前） | `/gd-review-plan` | 还没写代码，先审计划本身够不够具体、有没有"填表式"空话 | **双镜头审查 + 双模型有界修复**——Claude 自审 + Codex 两视角挑刺 → 合并 → 自动修复（≤3 轮），两边点头才过 |
| **③ 执行** | `/gd-exec` | 按定稿计划干活；执行只照单做，可用便宜模型 | **多 agent 编排**——派 1-2 分身并行做，每批做完查一次有没有越界 |
| **④ 审查**（动手后） | `/gd-review` | 对产出做最终审查 | **自动路由 + 双镜头审查**——自动识别审代码/执行结果/两者；两者一起时先验结果、过了才审代码 |

一眼看清机制落在哪：

- **多 agent 编排** → ① 计划、③ 执行（分身并行干活）。
- **双镜头机制** → ② 审计划、④ 审查：Codex 一个视角盯「结构 + 目标符合性」，另一个专挑「边界条件 + 漏报风险」。
- 四个阶段共守一条硬约束：每步 1-2 个子 agent、**并行上限 2**。

> 命令三种写法等价：顶层快捷 `/gd-plan`（插件下 `/project-gd:gd-plan`）= 两段式 `/gd plan` = 中文 `/gd 计划`；`/gd-exec` 的中文别名为 `/gd-执行`。
